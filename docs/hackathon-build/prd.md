...
4. **Independent agents, shared rails:** Belay exposes context and conflicts but does not assign work or sequence prompts.
...
1. The developer points Belay at a repository.
2. Belay validates the path, creates local state, indexes the repository, and opens the dashboard.
...
2. Belay returns the lease and shows the locked files in the cockpit.
...
3. Belay shows the command template, policy classification, environment variable names, and masked values.
...
4. The developer approves or rejects once. Belay records the immutable decision and either runs the configured disposable/local adapter or returns the rejection.

### Cloud intelligence path

1. The developer requests a semantic repository or audit summary.
2. Belay previews or records the sanitized payload category and applies the egress policy.
...
As a developer, I want to initialize Belay for a repository so that all agents use the same local control-plane state.
...
- **PRD-INIT-03:** Restarting Belay for an initialized repository restores stages, tasks, active non-expired locks, manifest metadata, and audit history without manual recovery.
...
- **PRD-IDX-05:** If the manifest exceeds its budget, Belay applies documented deterministic truncation priorities and reports what was omitted.
...
- **PRD-CMD-02:** Before execution, Belay reports the normalized executable, masked arguments, policy class, timeout, working-directory boundary, and environment variable names.
...
- Belay does not choose an agent, assign work automatically, or author model prompts.
- Belay does not expose secret retrieval APIs; all secret use is execution-scoped.
- Belay does not promise a universal shell sandbox or protection from a malicious local operating-system administrator.
- Belay does not send complete repositories to Gemini.
- Belay does not claim measured token savings beyond the reproducible fixture and documented estimator.
- Belay does not support production multi-tenant cloud storage in the hackathon MVP.
